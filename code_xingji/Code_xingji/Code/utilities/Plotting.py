...
def temp_error_in_meter_plotcdf():
    fn = 'interim_output/test_output_hmm.txt'
    # experiment = ["classification", "regression"]
    target_and_neural_out = np.loadtxt(fn, delimiter=',')

    error_in_meters = transfer_error_in_meters(target_and_neural_out, flag=True)
    with open("./interim_output/e_hmm.txt", "w") as f:
        np.savetxt(f, error_in_meters, delimiter=",", newline='\n')

    fig = plt.figure()
    data = np.loadtxt("./interim_output/e_hmm.txt")
    cdf_plot(data, "HMM", 100)

    plt.legend(loc=8, bbox_to_anchor=(0.65, 0.3), borderaxespad=0.)
    plt.show()
    fig.savefig("CDF_hmm.png")
    print("\n")
...
def main(which_kind_of_model):
    ...
    for suffix, model_name in zip(fn_suffix_list[0:2], models):
        data = np.loadtxt("../../Experiments/errors/errors_{}_{}.txt".format(which_kind_of_model, suffix))
...
def main1():
    ...
    for suffix, model_name in zip(fn_suffix_list, models):
        data = np.loadtxt("../../Experiments/comparison1/e_{}.txt".format(suffix))
        cdf_plot(data, model_name, 100)
...
def main2():
    ...
    for suffix, model_name in zip(fn_suffix_list, models):
        data = np.loadtxt("../../Experiments/comparison2/e_{}.txt".format(suffix))
        cdf_plot(data, model_name, 100)
...
def main3():
    ...
    for suffix, model_name in zip(fn_suffix_list, models):
        data = np.loadtxt("../../Experiments/comparison3/e_{}.txt".format(suffix))
        cdf_plot(data, model_name, 100)
